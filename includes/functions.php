...
function generate_pdf($order, $car){
    ...
    require(getcwd().'../includes/fpdf181/fpdf.php');

    $pdf = new FPDF('P','mm','A4');
	$pdf->AddPage();
	$pdf->Image(getcwd().'../includes/temp_images/szerzodes_old1.jpg', 0, 0, 210, 297); 
	...
	$pdf->AddPage();
	$pdf->Image(getcwd().'../includes/temp_images/szerzodes_old2.jpg', 0, 0, 210, 297); 
    ...
    $filename = 'berleti_szerzodes_ON000'.$order['rent_id'];
    $pdf->Output('F', getcwd().'/pdfs/'.$filename.'.pdf');		
    $pdf_url = 'http://localhost/api-server-teszt/pdfs/'.$filename.'.pdf';
    return $pdf_url;
